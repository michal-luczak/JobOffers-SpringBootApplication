...
import pl.luczak.michal.controller.error.APIValidationErrorDTO;
...
//        tokenBadRequestResult.andExpect(status().isUnauthorized());
    ...
    //step 4: user made GET /offer with no jwt token and system returned UNAUTHORIZED(401)
        ...
        //when
        ResultActions offerBadRequestResult = mockMvc.perform(get("/offer/" + uniqueID)
                .contentType(MediaType.APPLICATION_JSON)
        );
        ...
        Assertions.assertTrue(result.messages().contains("TestMessage"));
